...
Per-request orchestration code should be well under 1K characters — basically `validate → confirm → build_mcp_params → [invoke MCP, page on truncated, merge_rows] → apply_post_filters (guard) → rank → format_excel → markdown_table → draft_email → format_feedback`. No expansion loop; below-target results trigger a broker ask in the email, not a re-query.
