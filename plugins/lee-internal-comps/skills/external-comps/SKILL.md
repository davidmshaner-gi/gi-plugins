...
1. **Parse** the broker's paste into a request dict (see Input Contract below). Write it to
   `request.json` in the working folder.
2. **Run the plan command, never hand-derive the calls:**

   ```
   python3 helpers.py plan request.json
   ```

   It prints one JSON object: `tool_name`, `params_list` (the exact MCP calls, one per
   entry), `post_filter_counties`, `validated`, `missing_required`, `applied_defaults`,
   `warnings`, `error`. This is `validate_request` + `build_mcp_params` in one step, and
   the JSON it prints is the ONLY source of the parameters you send. Do not compute the
   date window, the price floor, the county list, or the limit yourself; do not "read the
   registry and build the calls" (gi-plugins#168: a Sonnet session did exactly that,
   landed 5 days off the helper's window, dropped the max date, and used a limit under
   the floor). Paste the printed entries into the tool calls verbatim.
3. If `missing_required` is non-empty (the command exits 1 with `error` set): draft a
   clarifying reply, stop. Do not call MCP. If `error` names an **unregistered market**
   (`UnknownMarket`: the broker said a market the registry does not know), ask the broker
   which counties or cities they mean and re-run the plan with `geography={"counties":
   [...]}` or `{"cities": [...]}`. **A named market never runs statewide.**
...
5. The plan's `params_list` holds one params dict per MCP call: one per county for a `counties` ask **and for `named_market: "RDU MSA"` (its 7 counties; every spelling of the Triangle, RDU, or Raleigh-Durham normalizes to it)**, one per city for `cities`. **Helpers do NOT call MCP.** You invoke `tool_name` directly, once per entry, with that entry's dict exactly as printed, and union the rows with `merge_rows(*pages)`. (`build_mcp_params(validated)` is the same function the command wraps; it also accepts the `validate_request` return value directly.)
...
10. Call `markdown_table(top, tagged_under_contract, tagged_sublet, tagged_rent_undisclosed, validated, applied_filters)` → returns a Markdown string for the chat reply (it refuses, like the Excel and the email, while the stale-connector notice is in `applied_filters`).
...
3. **If the broker uses a term you don't recognize** ("IOS," internal nicknames, a market the plan command rejects with `UnknownMarket`), ask them to describe what it maps to in the platform's terms. (Any spelling of the Triangle / RDU / Raleigh-Durham IS recognized; the plan command normalizes it.) Don't translate or guess. This rule applies to broker-internal shorthand, not the external platform's own labels.
...
Per-request orchestration code should be well under 1K characters — basically `plan → confirm → [invoke MCP, page on truncated per the step-6 algorithm — never window-slice, merge_rows] → apply_post_filters (guard) → rank → format_excel → markdown_table → draft_email → format_feedback`. No expansion loop; below-target results trigger a broker ask in the email, not a re-query.
