...
- **vpd-lookup reports the count at the site, with its NCDOT station ID (lee-and-associates#603).**
  ...
  every station on the named road, nearest first (and says "nearest N of M" when the tool's 10-row cap bites). The skill presents `value` exactly as returned
  (no rounding) and shows the station ID on each row so a broker can check it against NCDOT's map.
  Ships with lee-raleigh-mcp 0.62.0. Reported by Susan Fulton, 2026-09-24.
...
- **An RDU-market external comps ask no longer loses 22-58% of its comps (gi-plugins#158).**
  ...
  param). The Python whitelist stays on only as a stale-connector guard (G26): it drops nothing
  when the Worker honoured `county`, and a "dropped N rows" note on an RDU ask is the signal to
  refresh the connector's tools list. The null-county strategy dialog and the city → county fallback
  map went with the post-filter they belonged to: the Worker now derives a county geometrically for
  the 265 external rows the platform shipped without one (lee#515, Worker 0.53.0), so a county
  filter reaches them and the skill never fabricates a county for display.
- **A capped search is paged and named, never passed off as complete.** Worker 0.53.0 puts
  ...
  cursor (`next_page_params`, up to `MAX_PAGES` = 5), unions with `merge_rows`, and appends one
  labelled `truncation_note` per clipped call so the email ("Notes on what was retrieved") and the
  Methodology sheet ("Retrieval notes") say how much of the matching book the broker is looking at.
